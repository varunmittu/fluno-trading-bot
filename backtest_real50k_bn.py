...
def run(sym, unit_lot, base_lots, max_lots, cap_per_lot, dte, iv, label):
    ...
    for d in sorted(day_net):
        ...
        m["net"] += day_net[d]
        if day_net[d] > 0: m["g"] += 1
        elif day_net[d] < 0: m["r"] += 1
    eq = START_CAPITAL
    print("\n-- " + label)
    for key in months:
        eq += months[key]["net"]
        print(f"   {key:>9} | {months[key]['g']:>2}g/{months[key]['r']:>2}r | P&L Rs.{months[key]['net']:>+10,.2f} | capital Rs.{eq:>10,.2f}")
    wins = [x for x in trades if x > 0]
    wr = len(wins)/len(trades)*100 if trades else 0
    g = sum(1 for v in day_net.values() if v > 0); rr = sum(1 for v in day_net.values() if v < 0)
    print(f"   lot {unit_lot}u | DTE {dte}d | trades {len(trades)} | win {wr:.0f}% | green/red days {g}/{rr}")
    print(f"   NET Rs.{capital-START_CAPITAL:+,.0f} -> final Rs.{capital:,.0f}")
    return capital

print("Fetching data + running (this takes ~1 min)...")
print("=" * 74)
print("PRACTICE Apr10-Jul3 2026 | Rs.50,000 start, profits reinvested into lots")
print("=" * 74)
# NIFTY sanity: 25-unit model, base 3, matches prior +Rs.13,422 ballpark
run("^NSEI",    65, 1, 15, 10000, 4,  0.14, "NIFTY — REAL 65u lot, weekly Tue expiry")
run("^NSEBANK", 30, 1, 15, 20000, 20, 0.13, "BANKNIFTY — REAL 30u lot, MONTHLY-only (Kite-confirmed)")
print("\n" + "=" * 74)
