...
def simulate(frame, t_from, t_to, capital, lots, trades_out, day_net):
    ...
    for d in [x for x in all_d if t_from <= x <= t_to]:
        _dte_mult = 1.41 if d.weekday() == 1 else 1.0   # Tue -> next-week contract
        ...
        for k, i in enumerate(idxs):
            ...
            if pos:
                ...
                _p1 = peak / _sc
                floor = None
                for _r in (150, 300, 500, 700, 850, 900):
                    if _p1 >= _r: floor = _r
                    else: break
                if floor == 900 and _p1 >= 1050:
                    floor = 900 + 150 * int((_p1 - 900) // 150)
                if floor: floor *= _sc
                if t >= dtime(15, 25) or last:                 booked = (round(pnl, 2), "EOD")
                elif floor and pnl < floor:                    booked = (round(floor, 2), "STEP")
                elif not floor and pnl <= -pos["sl"]:          booked = (-pos["sl"], "SL")
                if booked:
                    val, status = booked
            ...
            if not (dtime(10, 15) <= t <= dtime(15, 0)):
                pending = None; continue
...
print("\n" + "=" * 74)
print("v4.4 FINAL 1 Jan -> 6 Jul 2026 | Rs.15,000 | staircase+fade0.2+Tue+15:00 window")
