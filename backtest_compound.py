"""
FULL-COMPOUND RUN - 1 Jan -> 3 Jul 2026, v4.2 rules but:
  ALL capital deployed in lots every trade (no 15-lot cap, no loss reset)
  risk rules scale with capital (SL, BE, trails, daily limit = same % as at 10K)
...
def simulate(frame, t_from, t_to, capital, lots, trades_out, day_net):
    ...
    for d in [x for x in all_d if t_from <= x <= t_to]:
        if d.weekday() == 0: continue          # expiry Monday skip
        ...
        day_hi_run = -1e18
        scale     = max(1.0, capital / START_CAPITAL)   # risk grows with capital
        sl_lo_d, sl_hi_d = SL_MIN*scale, SL_MAX*scale
        be_d      = BE_START*scale
        sm_st_d, sm_dr_d = SMALL_START*scale, SMALL_DROP*scale
        big_st_d, big_sf_d = BIG_START*scale, BIG_SAFETY*scale
        dlim_d    = DAILY_LIMIT*scale
        ...
        for k, i in enumerate(idxs):
            ...
            if pos:
                ...
                peak = pos["peak"]
                if peak >= be_d: pos["locked"] = True
                booked = None
                if t >= dtime(15, 25) or last:                 booked = (round(pnl, 2), "EOD")
                elif pos["locked"] and pnl < be_d:             booked = (be_d, "BE")
                elif not pos["locked"] and pnl <= -pos["sl"]:  booked = (-pos["sl"], "SL")
                elif peak >= big_st_d:
                    if not trend_strong(frame, i, pos["otype"]) or pnl <= peak-big_sf_d:
                        booked = (round(pnl, 2), "TRAIL")
                elif peak >= sm_st_d and pnl <= peak-sm_dr_d:
                    v = max(be_d, round(pnl, 2)) if pos["locked"] else round(pnl, 2)
            ...
            def try_open(otype, sig, conf, sl):
                ...
                use = afford      # ALL capital, every trade
            ...
            sl = max(sl_lo_d, min(sl_hi_d, sl * scale))
            if trades_today == 0:
...
print("\n" + "=" * 74)
print("PRACTICE RUN 1 Jan -> 3 Jul 2026 | TODAY'S SETTINGS (v4.2, no limits)")
