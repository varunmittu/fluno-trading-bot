"""
backtest_reduce_losses.py (2026-07-27) — Sai: "can we reduce losses without
hurting wins?" Test loss-reduction filters on 6 MONTHS of REAL Kite 5-min data.
A variant only "works" if it keeps NET P&L ~same-or-better (wins intact) while
cutting losses / drawdown. If net drops, the filter killed wins too.
Variants: tighter ADX chop line, re-added SL cooldown, higher score gate, combos.
"""
import sys, os, re
...
NIFTY50 = 256265
end = datetime.now(); start = end - timedelta(days=182)
rows, cur = [], start
while cur < end:
...
print(f"6-month real data: {f['day'].nunique()} trading days\n")
...
def run(adx_min=15, cooldown=0, min_score=7.0, use72=True):
    cfg = dict(be.NEW); cfg["cooldown"] = cooldown
    dr = {d: f.index[f["day"] == d].tolist() for d in f["day"].unique()}
    alld = sorted(f["day"].unique()); trades, day_net = [], {}
    for d in alld:
        ...
        for kk, i in enumerate(idxs):
            ...
            gate = (c9>=min_score or cp>72) if use72 else (c9>=min_score)
            if not gate: continue
            if not pd.isna(row["adx"]) and row["adx"]<adx_min: continue
            a=row["atr14"]
            if np.isnan(a): continue
            sl=max(SL_MIN,min(SL_MAX,round(a*cfg["atr_mult"]*DELTA*UNITS/10)*10))
            pos={"otype":otype,"entry":px,"sl":sl,"peak":-9e9,"inv":round(be.premium_est(px)*dte*UNITS,2)}
    nets=trades; wins=[n for n in nets if n>0]; losses=[n for n in nets if n<=0]
    eq=pk=mdd=0.0
    for dd in sorted(day_net):
        eq+=day_net[dd]; pk=max(pk,eq); mdd=max(mdd,pk-eq)
    return dict(net=sum(nets), n=len(nets), wr=len(wins)/len(nets)*100 if nets else 0,
                nloss=len(losses), avg_loss=sum(losses)/len(losses) if losses else 0,
                avg_win=sum(wins)/len(wins) if wins else 0, mdd=mdd,
                worst=min(day_net.values()) if day_net else 0)

variants = [
    ("BASELINE (live: ADX15, no cooldown)", dict()),
    ("ADX >= 20 (skip more chop)",          dict(adx_min=20)),
    ("ADX >= 25 (skip most chop)",          dict(adx_min=25)),
    ("30-min cooldown after a stop",        dict(cooldown=30)),
    ("Score gate >= 7.5",                   dict(min_score=7.5)),
    ("ADX>=20 + 30-min cooldown",           dict(adx_min=20, cooldown=30)),
]
print(f"{'variant':<38}{'net':>10}{'trades':>7}{'win%':>6}{'#loss':>6}{'avgWin':>8}{'avgLoss':>8}{'maxDD':>8}")
for name, kw in variants:
    r = run(**kw)
    print(f"{name:<38}{r['net']:>+10,.0f}{r['n']:>7}{r['wr']:>5.0f}%{r['nloss']:>6}"
          f"{r['avg_win']:>+8.0f}{r['avg_loss']:>+8.0f}{r['mdd']:>8,.0f}")
print("\nRule: a filter only WORKS if 'net' stays ~same/higher while #loss or maxDD drops.")
print("If net falls, the filter killed wins too — not worth it.")
