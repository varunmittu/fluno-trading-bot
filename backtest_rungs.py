"""
STAIRCASE RUNG LADDER test — Sai 2026-07-08: "add 150,250,300,450, so on".
Denser rungs = lock smaller profits sooner, give back less on reversals,
but also book winners earlier. Tested on the LIVE config as of now:
ATR 1.0x SL (Rs.200-1000/lot), NO daily limit, fixed 1 lot, real charges.

S0 = CURRENT     150,300,500,700,850,900 then +150
S1 = SAI LITERAL 150,250,300,450 then +150 (600,750,900,...)
S2 = SAI DENSE   150,250,300,450,500,700,850,900 then +150
S3 = EVEN 150s   150,300,450,600,750,900 then +150
"""
import os, math
...
def make_rungs(base):
    return list(base) + list(range(base[-1]+150, 300001, 150))

LADDERS = {
    "S0 CURRENT     150,300,500,700,850,900,+150": make_rungs([150, 300, 500, 700, 850, 900]),
    "S1 SAI LITERAL 150,250,300,450,+150":         make_rungs([150, 250, 300, 450]),
    "S2 SAI DENSE   150,250,300,450,500,700,850,900,+150": make_rungs([150, 250, 300, 450, 500, 700, 850, 900]),
    "S3 EVEN 150s   150,300,450,600,750,900,+150": make_rungs([150, 300, 450, 600, 750, 900]),
}
...
def simulate(rungs):
    ...
    return dict(net=sum(trades), n=len(trades), wr=wr, worst_day=worst_day,
                mdd=mdd, green=g, red=rr, avg_win=np.mean(wins) if wins else 0)

d0, d1 = df5["day"].min(), df5["day"].max()
print("\n" + "=" * 78)
print(f"STAIRCASE LADDER TEST | {d0} -> {d1} | live config: ATR SL, NO daily limit, 1 lot")
print("=" * 78)
for name, rungs in LADDERS.items():
    r_ = simulate(rungs)
    print(f"\n{name}")
    print(f"   NET Rs.{r_['net']:+,.2f} | trades {r_['n']} | win {r_['wr']:.0f}% | green/red {r_['green']}/{r_['red']}")
    print(f"   avg win Rs.{r_['avg_win']:+,.0f} | worst day Rs.{r_['worst_day']:+,.0f} | max drawdown Rs.{r_['mdd']:,.0f}")
print("\n" + "=" * 78)
