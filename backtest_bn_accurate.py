"""
Accurate, parameterized v4.2 backtest engine — ready to plug REAL Kite params.
Runs NIFTY (sanity) vs BANKNIFTY with correct per-instrument lot size and a
premium model that takes days-to-expiry (weekly=4 vs monthly=~18) so we can
see how much the weekly-vs-monthly question matters for BANKNIFTY.

PROVISIONAL until tomorrow's live Kite data confirms: BN lot size, expiry
cycle (weekly/monthly), and real ATM premiums. Numbers here are estimates.
Last 60 days, 5-min data, real Zerodha charges.
"""
import os, math
...
def get_yf(sym, interval, period):
    d = yf.download(sym, interval=interval, period=period, progress=False).reset_index()
...
def run(sym, unit_lot, base_lots, max_lots, cap_per_lot, dte, iv, label):
    frame = prep(get_yf(sym, "5m", "60d"))
    # 15-min trend filter
    r = get_yf(sym, "5m", "60d").set_index("dt")
    ...
    htf = pd.merge_asof(frame[["dt"]].sort_values("dt"),
                        f15[["usable_from","st_dir","macd_up15"]].sort_values("usable_from"),
                        left_on="dt", right_on="usable_from", direction="backward")
    frame["htf_st"] = htf["st_dir"].values; frame["htf_macd"] = htf["macd_up15"].values
    ...
    def premium_est(spot):
        return round(spot * iv * math.sqrt(dte/365) * 0.4 * 0.98, 1)
    ...
    for d in all_d[1:]:
        if d.weekday() == 1: continue          # Tuesday expiry skip (corrected)
        ...
        for k, i in enumerate(idxs):
            ...
            if pos:
                ...
                if booked:
                    val, status = booked; gross = val + MODEL_BROKERAGE
                    sell_val = max(0.0, pos["inv"] + gross); chg = real_charges(pos["inv"], sell_val)
                    net = round(gross - chg, 2); capital += net; daily_pnl += net
            ...
            def try_open(otype, conf, sl):
                nonlocal pos
                prem = premium_est(px); afford = int(capital // (prem*unit_lot))
                if afford < 1: return
                use = min(lots, afford)
                pos = {"otype": otype, "entry": px, "sl": sl, "lots": use,
                       "peak": -9999, "locked": False, "inv": round(prem*use*unit_lot, 2)}
            ...
            if not htf_ok(i, otype): continue
    ...
    print(f"\n-- {label}")
    print(f"   lot {unit_lot}u | premium DTE {dte}d IV {iv:.0%} | trades {len(trades)} | win {wr:.0f}% | "
          f"green/red {g}/{rr}")
    print(f"   NET Rs.{capital-START_CAPITAL:+,.0f} -> final Rs.{capital:,.0f}  "
          f"[PROVISIONAL — needs real Kite lot/premium/expiry]")
    return capital

print("Fetching data + running (this takes ~1 min)...")
print("=" * 74)
print("ACCURATE-ENGINE v4.2 | NIFTY sanity vs BANKNIFTY (weekly vs monthly)")
print("=" * 74)
# NIFTY sanity: 25-unit model, base 3, matches prior +Rs.13,422 ballpark
run("^NSEI", 25, 3, 15, 10000/3, 4, 0.14, "NIFTY (25u model, weekly) — sanity check")
# BANKNIFTY: 15-unit lot, base 1, max 5. Weekly (if it existed) vs Monthly.
run("^NSEBANK", 15, 1, 5, 5000, 4,  0.13, "BANKNIFTY — IF weekly options (DTE 4)")
run("^NSEBANK", 15, 1, 5, 5000, 18, 0.13, "BANKNIFTY — monthly options (DTE 18, likely real)")
print("\n" + "=" * 74)
print("NOTE: BANKNIFTY rupee figures are ESTIMATES. Confirm tomorrow via live")
print("Kite: (1) lot size (2) weekly or monthly expiry (3) real ATM premium.")
print("=" * 74)
