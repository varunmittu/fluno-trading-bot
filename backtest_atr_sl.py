...
ATR_MULT = None   # None = conf-based SL; number = uncapped ATR SL
def simulate(mode):
    frame = df5
    ...
    for d in all_d[1:]:
        if d.weekday() == 1 and not TRADE_TUESDAY: continue   # expiry skip unless enabled
        _dte_mult = 1.41 if d.weekday() == 1 else 1.0   # next-week premium ~ sqrt(8/4)
        ...
        for k, i in enumerate(idxs):
            ...
            if pos: continue
            if not (dtime(10,15) <= t <= WINDOW_END): pending = None; continue
            if cooldown and row["dt"] < cooldown: continue

            def try_open(otype, conf, sl):
                nonlocal pos
                prem = premium_est(px) * _dte_mult; afford = int(capital // (prem*UNITS))
                if afford < 1: return
                use = min(lots, afford)
            ...
            if ATR_MULT:
                _a = frame.loc[i, "atr"]
                if not np.isnan(_a):
                    sl = max(100, round(ATR_MULT * _a * DELTA * UNITS))   # NO upper cap
